...
# Description: FSK9600 AX.25 decoder with G3RUH support adjusted for SKCUBE1
# Generated: Thu Jul 19 10:58:54 2018
##################################################
...
class satnogs_fsk9600_g3ruh_ax25_skcube(gr.top_block):

    def __init__(self, antenna=satnogs.not_set_antenna, bb_gain=satnogs.not_set_rx_bb_gain, decoded_data_file_path="/tmp/.satnogs/data/data", dev_args=satnogs.not_set_dev_args, doppler_correction_per_sec=1000, file_path="test.wav", if_gain=satnogs.not_set_rx_if_gain, iq_file_path="/tmp/iq.dat", lo_offset=-85e3, ppm=0, rf_gain=satnogs.not_set_rx_rf_gain, rigctl_port=4532, rx_freq=100e6, rx_sdr_device="usrpb200", udp_IP="127.0.0.1", udp_port=16887, waterfall_file_path="/tmp/waterfall.dat", enable_iq_dump=1):
...
def argument_parser():
    ...
    parser.add_option(
        "", "--lo-offset", dest="lo_offset", type="eng_float", default=eng_notation.num_to_str(-85e3),
...
def main(top_block_cls=satnogs_fsk9600_g3ruh_ax25_skcube, options=None):
    if options is None:
        options, _ = argument_parser().parse_args()

    tb = top_block_cls(antenna=options.antenna, bb_gain=options.bb_gain, decoded_data_file_path=options.decoded_data_file_path, dev_args=options.dev_args, doppler_correction_per_sec=options.doppler_correction_per_sec, file_path=options.file_path, if_gain=options.if_gain, iq_file_path=options.iq_file_path, lo_offset=options.lo_offset, ppm=options.ppm, rf_gain=options.rf_gain, rigctl_port=options.rigctl_port, rx_freq=options.rx_freq, rx_sdr_device=options.rx_sdr_device, udp_IP=options.udp_IP, udp_port=options.udp_port, waterfall_file_path=options.waterfall_file_path, enable_iq_dump=options.enable_iq_dump)
    tb.start()
    tb.wait()
